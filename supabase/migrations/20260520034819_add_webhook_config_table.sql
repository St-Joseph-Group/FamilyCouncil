...
/*
  ...
  1. New Tables
    - `webhook_configs` - Stores n8n and other webhook endpoint configurations
      - `id` (uuid, primary key)
      - `name` (text) - friendly label e.g. "n8n Facebook Messenger"
      - `url` (text) - the webhook endpoint URL
      - `is_active` (boolean) - whether this webhook is enabled
      - `last_tested_at` (timestamptz) - when the connection was last tested
      - `last_status` (text) - "connected" | "disconnected" | "error"
      - `last_status_message` (text) - error or success message from last test
      - `headers` (jsonb) - optional custom headers
      - `created_by` (uuid)
      - `updated_by` (uuid)
      - timestamps

  2. Security
    - RLS DISABLED per project requirement (global flag: rls_enabled = false)
